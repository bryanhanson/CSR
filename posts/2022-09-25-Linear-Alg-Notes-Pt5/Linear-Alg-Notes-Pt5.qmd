...
B --> E(upper triangular matrices)
B --> F(lower triangular matrices)
B ---> G{{is singular?}}
B ---> H{{is invertible?}}
...
| structure | special cases | condition | properties | decomposition | eigenvalues |
|:-|:-|:-|:-|:-|:-|:-|
| all matrices $n \times m$ | | | | SVD| |
...
| $n \times n$ con't | | **either:** is singular | $\det(\mathbf{A}) = 0$ | QR, LU, eigen | $\text{at least one } \lambda = 0$[^5] |
| | | **or:** is invertible | $\det(\mathbf{A}) \ne 0$ | QR, LU, eigen |  $\text{all } \lambda \ne 0$|
...
| | symmetric | | $\mathbf{A} = \mathbf{A}^\mathsf{T} \quad a_{ij} = a_{ji}$[^1] | | $\text{ all } \lambda \in \mathbb{R}$ |
| | diagonal | | $a_{ij} = 0 \text{ if } i \ne j$ | |  $\lambda_i = a_{ii}$ |
| | | is orthogonal | $\mathbf{A}^{-1} = \mathbf{A}^\mathsf{T} \quad \mathbf{A}\mathbf{A}^\mathsf{T} = \mathbf{A}^\mathsf{T}\mathbf{A} = \mathbf{I}$ | | $\text{all } \lvert\lambda\rvert = 1$|
| | identity | | $\mathbf{A}\mathbf{I} = \mathbf{I}\mathbf{A} = \mathbf{A}$ | | $\text{all } \lambda = 1$ |

{{< pagebreak >}}

The decompositions and their common uses:

* SVD = singular value decomposition. $\mathbf{\mathbf{A}} = \mathbf{U}\mathbf{\Sigma}\mathbf{V}^{\mathsf{T}}$.  A key use is for PCA = Principal Components Analysis.  For more info see [here](https://chemospec.org/posts/2022-05-03-LearnPCA-Intro/2022-05-03-LearnPCA-Intro.html).
* Eigen decomposition.
  + Can be used for PCA if one centers the data and computes $\mathbf{A}^{\mathsf{T}}\mathbf{A}$, the correlation matrix (which will have dimensions $n \times n$).
* QR. $\mathbf{A} = \mathbf{Q}\mathbf{R}$
* LU. $\mathbf{A} = \mathbf{L}\mathbf{U}$.[^3]  A common use is to solve systems of simultaneous linear equations $\mathbf{A}\mathbf{x} = \mathbf{b}$, including linear regression.
