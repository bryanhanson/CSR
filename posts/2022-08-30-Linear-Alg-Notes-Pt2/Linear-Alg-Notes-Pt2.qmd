...
The key point here is that once again we need to invert a matrix to solve this.  The details of where @eq-9 comes from are covered in a number of places, but I will note here that $\hat{\beta}$ refers to the best estimate of $\beta$.
...
[^7]: We could also write this as $\mathbf{x} = (x, y, z)^\mathsf{T}$ to emphasize that it is a column vector. One might prefer this because the only vector one can write in a row of text is a row vector, so if we mean a column vector many peoplee would prefer to write it transposed.
