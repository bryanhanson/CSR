...
<img src="/images/Manual_Alignment.png" alt="Manual Alignment" style="width:400px;"/>
...
The actual numerical values are in `cos_dist`, a matrix.  Looking at the heatmap, there are some modest differences among the spectra. However, if one looks at the scale, cosine distances are only defined on [-1 ... 1].  While the cosine distance is popular in some spectroscopic contexts, it's not the best objective function because there is little absolute difference between -1 and 1 (and for `MUD2` the absolute differences are even smaller, as the range is only `r round(range(cos_dist), 2)`).
...
In the next post I'll discuss the general flow of the HATS-PR algorithm, and how to carry it out using `ChemoSpec2D`.
